...
public class RunIterator implements Iterator {
  ...
  private DListNode runNode;
  private DListNode head;

  /**
   *  RunIterator() constructs a new iterator starting with a specified run.
   *
   *  @param runList the run where this iterator starts.
   */
  ...
  // the encoding.
  RunIterator(DList runList) {
    // is it alright to add a __getHead__ method to DList class?
    head = runList.getHead();
    runNode = head.next;
  }
  ...
  public int[] next() {
    ...
    // readme file said 'a new array' that 'not in the RunLengthRncoding data structure'

    /* DO CHECK hasNext() everytime you use next() */
//    if (hasNext()) {
//      int[] i = (int[]) runNode.item;
