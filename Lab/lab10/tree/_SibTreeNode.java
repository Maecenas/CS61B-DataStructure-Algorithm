...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    if (isValidNode()) {
      if (c <= 1) {
        SibTreeNode newSibTreeNode = new SibTreeNode(myTree, item);
        newSibTreeNode.parent = this;
        newSibTreeNode.nextSibling = firstChild;
        firstChild = newSibTreeNode;
      } else {
        int numChildren = children();
        SibTreeNode newSibTreeNode = new SibTreeNode(myTree, item);
        newSibTreeNode.parent = this;
        SibTreeNode childNode = firstChild;
        ...
        if (childNode == null) {
          ...
          if (numChildren < c) {
            for (int i = 0; i < numChildren - 1; i++) {
              // childNode at the last child
              childNode = childNode.nextSibling;
            }
            childNode.nextSibling = newSibTreeNode;
          } else {
            for (int i = 0; i < c - 2; i++) {
              // childNode at the (n - 1)th node
              childNode = childNode.nextSibling;
            }
            newSibTreeNode.nextSibling = childNode.nextSibling;
            childNode.nextSibling = newSibTreeNode;
          }
        }
      }
      ...
      if (firstChild == null) {
        if (this.equals(myTree.root)) {
          ...
          if (this.equals(parent.firstChild)) {
            ...
            SibTreeNode childNode = parent.firstChild;
            while (!(this.equals(childNode.nextSibling))) {
              childNode = childNode.nextSibling;
            }
